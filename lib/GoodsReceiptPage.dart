...
import 'GoodsReceiptListPage.dart';
import 'dart:convert';
...
class _GoodsReceiptPageState extends State<GoodsReceiptPage> {
  ...
  List<Map<String, dynamic>> recentGRNs = [];
  ...
  Future<void> loadDashboard() async {
    final response = await http.get(
      Uri.parse("${AppConfig.apiBaseUrl}/api/grn/dashboard"),
    );
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        dashboard = json["data"] ?? {};
        recentGRNs = List<Map<String, dynamic>>.from(
  ...
  double get todayPOValue {
    ...
    for (final po in recentGRNs) {
      if (po["grandTotal"] == null || po["receivedDate"] == null) continue;

      final date = DateTime.parse(po["receivedDate"]);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "Here's your goods receipt overview",
            ...
            GridView.count(
              ...
              children: [
                ...
                _actionButton(Icons.list_alt, "GRN List", Colors.blue, () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) =>
                          GoodsReceiptListPage(companyId: widget.companyId),
                    ),
                  );
                }),
